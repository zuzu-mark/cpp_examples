...
void Logger::SetFileName(const char *filename) { filename_ = filename; }
...
void Logger::AddToQueue(LOGLEVEL loglevel, const char *pszFile, int lineNo,
    ...
    tm *tmstr;
    // tm* tmstr=&tmstr_;
    // struct tm* tmstr = localtime_r(&now);
    // struct tm* tmstr = localtime_r(&now);
    // localtime_r(&now,tmstr);
    {
        std::lock_guard<std::mutex> guard(mutex_);
        tmstr = std::localtime(&now);
    }
    char *logLevel;
    ...
    sprintf(content, "[%04d-%02d-%02d %02d:%02d:%02d][%s][0x%04x][%s:%d %s]%s",
            tmstr->tm_year + 1900, tmstr->tm_mon + 1, tmstr->tm_mday,
            tmstr->tm_hour, tmstr->tm_min, tmstr->tm_sec, logLevel,
            std::this_thread::get_id(), pszFile, lineNo, pszFuncSig, msg);
#endif
...
void Logger::setLogTargetRule(uint8_t rule) { log_target_rule_ = rule; }

void Logger::setLogLevelRule(uint8_t rule) { log_level_rule_ = rule; }
